...
点击主线卡立即设为当前主线。主线和事项文字均原地编辑。事项行展示“事项名称 ｜ 最小一步”，只有填写最小一步时才显示分隔符；空字段通过轻量入口添加。添加区可显式选择“持续”，持续事项以低存在感标签显示累计次数。事项整行拖动可排序、改归属或设为下一步；一次事项末尾 `✓` 表示完成，持续事项末尾 `✓` 表示今天完成一次。右键可把一次事项设为持续事项；持续事项可撤回今天、达成并结束、不再继续或 hard delete。主线右键完成、放弃或删除。系统永不按创建时间或日期自动重排。
