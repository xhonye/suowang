...
行动中可见性补充（2026-09-08，覆盖上文间歇动效节奏）：下一步以常驻「正在走这一步」蓝色标记和卡片边框表达执行，箭头在 4.2 秒内持续向右移动，不再快速掠过后隐藏三秒。减少动态效果时保留文字与静态样式。WidBar 在任务栏预览右侧常驻「执行中」标记。两端每 3 秒同步，仅在数据变化时重绘驾驶舱，未保存编辑期间不得覆盖输入。
