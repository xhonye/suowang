...
事项只有 `active / completed / abandoned` 三种状态，统一为可持续做的事项，不再让用户选择单次／持续。内部仍使用稳定的 `todos` 表、Todo API 名与 `state_id`，但用户界面和对外产品语言不得暴露 Todo/状态 Todo/主线 Todo。`state_id` 不可变，`mainline_id` 可空；事项可以在同模式的其他事项区和任意主线之间移动并保留 ID，不能跨模式。事项名称必填；可选 `minimal_step` / 最小一步回答“现在怎样最低成本迈出去”，严格区别于主线的本阶段完成标准。所有事项点击“今天完成”只增加一条当日本地自然日记录，事项继续 active，同一天数据库最多接受一次。计数表示已做多少天，不计算连续天数、缺卡、完成率、积分或奖惩，也不提供提醒；通过更多菜单或右键的“结束事项”或“放弃事项”进入行迹；结束不增加记录，一次性的事情可今天完成后再结束。行迹中的事项提供纠错用的撤回：恢复为 `active` 并保留原 ID；原归属主线仍 active 时回到该主线，否则回到同模式其他事项。
...
点击主线卡立即设为当前主线。非当前主线不显示“进行中”等状态标签；当前卡主要依靠选中样式表达，并仅以低存在感文字补充“当前主线”。主线更多操作使用几何居中的 SVG 横向三点与圆形 hover 点击区。主线和事项文字均原地编辑。事项行展示“事项名称 ｜ 最小一步”，只有填写最小一步时才显示分隔符；空字段通过轻量入口添加。添加区只输入事项并添加，不显示类型选择：名称前不加属性标签，只在右侧完成按钮旁以低存在感的 `↻ N` 表示累计次数；今天完成只让 `✓` 进入安静选中态，不给整行染色。事项整行拖动可排序、改归属或设为下一步；所有事项末尾 `✓` 表示今天完成一次。事项更多菜单与右键共用生命周期操作，文案统一为“结束事项 / 放弃事项 / 删除事项”，当天有记录可撤回今天，不提供类型转换。主线右键完成、放弃或删除。系统永不按创建时间或日期自动重排。

下一步区域提供右上角低存在感的「卡住了？」入口。点击后调整面板原地接管整个下一步卡片，再点同一入口收拢；不开大弹窗、不新增页面，展开高度与普通下一步卡保持一致。为此可压缩“下一步”标题区，但必须保留四项完整解释。下一步卡片不重复显示所属主线，也不能作为事项被拖动。未开始时，卡片主操作为「开始这一步」；点击后持久化行动中指针，卡片显示「正在走这一步」，并把控制明确拆为「暂停」和完成操作。暂停只清除行动中指针，不结束、不改写也不记录事项。行动中动效覆盖整张下一步卡片（含标题区）：一组单一蓝色填充的粗人字箭头块每隔数秒快速向右掠过；每 4.2 秒以约 0.9 秒完成一轮掠过，其余时间隐藏；蓝色清楚可见，标题和控件附近淡出；箭头尖端完整置于重复图块内部，不使用双色描边、细线替代或渐变闪光。暂停、完成或下一步变更后停止，原地编辑时暂缓流动。卡片保持紧凑高度，文字与按钮不移动；减少动态效果时仅保留静态行动中样式。不修改道路背景、箭头或模式 Tab。面板不评判执行力，只允许用户调整四个维度：事情太难启动时缩小**步幅**（再小一点），此刻不适合时更换**动作**（换一件事），持续逃避时检查**方向**（看看主线），能量不足时切换**模式**（先去恢复）。这四项是既有模型的导航，不得扩张成新的业务实体、习惯系统或激励系统。
...
事项提供可见三点菜单，支持直接设为下一步、同模式移动及生命周期操作；移动放到目标列表末尾，菜单可用方向键、Home、End、Escape，退出后恢复焦点。「换一件事」与设为下一步菜单仅提供当前主线或其他事项中当天仍可做的事项；服务端选择和开始同样拒绝当天已完成的持续事项。

下一步为空时提供「添加第一步」，直接聚焦当前主线的添加框；没有当前主线时聚焦其他事项，不强迫先规划主线。当前主线与其他事项中的持续事项当日全部完成时，显示安静的当日完成说明与可选「再添一步」，不要求继续添加，不把其他主线或模式也算作完成。

原地修改保存失败时必须保留输入与焦点，允许修正重试；Enter 保存、Escape 取消后返回原控件，输入法组合输入期间不处理这两个快捷键。确认弹窗仅在请求成功后关闭，失败时在弹窗内保留字段并显示原因；提交期间防止重复请求。跨模式复制行迹后显示并保存目标模式，恢复备份后按备份中的模式重绘驾驶舱。

下一步为空但当前范围已有可做事项时，按既有当前主线／其他事项顺序展示一个提议，提供「选为下一步」并等待用户明确选择；只有确实没有可做事项时才引导添加。它不自动选择、不创建重复事项，也不替用户开始。浏览器与 Desktop 的设置页都显示实际服务版本。

行动中可见性补充（2026-09-08，覆盖上文间歇动效节奏）：下一步以常驻「正在走这一步」蓝色标记和卡片边框表达执行，箭头在 4.2 秒内持续向右移动，不再快速掠过后隐藏三秒。减少动态效果时保留文字与静态样式。WidBar 任务栏预览标题最多两行，超出省略、悬停显示全文；标题区域左右居中；右侧无框灰蓝图标在进行中显示暂停、其他可开始状态显示播放，点击直接暂停或开始／继续；「进行中／暂停中」保留在悬停提示，不新增业务状态。两端每 3 秒同步，仅在数据变化时重绘驾驶舱，未保存编辑期间不得覆盖输入。

事项可有可选 `notes` / 备注（最多 4000 字符），仅保存用户自己的补充想法；不作为最小一步、下一步或优先级依据。列表和下一步卡片不展示备注正文或徽标，通过事项菜单按需查看／编辑；完成或放弃后随事项保留，在行迹中只读查看。迁移、导出和整库恢复必须保留备注，保存失败保留输入。

## 数据与技术合同

事项统一模型兼容：数据库保留 `kind` 列供旧库／历史读取，但所有新事项和撤回后的事项统一为 `ongoing`；旧客户端显式传 `single` 创建时也归一为 `ongoing`，不重新引入类型选择。migration 009 只转换 active 事项，不改历史状态、ID、备注、排序或次数，不补造 `todo_occurrences`。既有 `/complete` API 保持稳定，对外语义为结束事项，不记一次完成。

Desktop 右上角关闭默认收起到系统托盘，保留本地服务和 WidBar 连接；设置页可切换为退出程序。托盘提供打开所往与退出所往，明确退出时释放实例锁并关闭服务，不能再次拦截为隐藏。托盘创建失败保持窗口可见并报错。关闭偏好作为本机桌面壳设置原子保存于数据目录 `desktop-preferences.json`，不改变业务 schema；浏览器/Lite 页面不显示该桌面专属设置。

WidBar 个人试用入口位于 `integrations/widbar/`：任务栏优先展示当前模式下一步的最小一步，无最小一步时显示事项名称；原生轻量卡片复用模式、事项和指针 API，支持编辑最小一步、开始／暂停、完成、换一件和空态添加其他事项。它不建立第二份数据库、不读写 SQLite、不增加打卡或催促机制。连接必须验证所选数据目录的实例锁、loopback 监听进程与 health PID；断线隐藏旧事项并明确显示未连接。未保存编辑固定关联原事项并保留到保存成功或明确取消；期间禁用上下文切换与事项执行，断线时仍可编辑或取消草稿。仅用户点击启动安装版，不后台反复拉起服务。此入口以减少行动成本为目标，不以打开次数为验收目标。
...
- `assets/brand/suowang-scenic-mark-v1.png`：界面和平台图标共同使用的山水道路 Logo 母版；`scripts/build-icons.mjs` 从它派生 PNG、ICO、favicon 和 macOS ICNS，不再从历史圆环箭头 SVG 生成。`scripts/refresh-shortcut-icons.ps1` 可备份并更新本机已有有效安装快捷方式的图标，保留启动目标与参数。
...
- 用户已明确要求常规界面调整直接实现与验证，不再把独立预览作为默认交付或必经步骤；当次用户明确要求评估／预览时再按其范围处理。
...
- 先锁定本轮范围，不把局部功能顺手扩成跨组件整治；若必须承接上轮未交付更新，先说明合并范围。
- 顺序固定为「实现完整 → 针对性验证与修复 → 检查最终 diff → 构建并验证最终包 → 安装更新」。界面、菜单顺序与交互尚在调整时，不提前制作安装包。
- 验证与风险相称：局部修复先跑受影响测试；需要全量回归时集中跑一次，未受后续改动影响的有效结果不重复跑。发现缺陷必须修复并重测受影响路径；“只打一次包”是避免浪费的目标，不是接受旧包或跳过验证的理由。
- 数据迁移、备份恢复、批准视觉资产、桌面安全和公开发行门禁不因提速而削弱；本机小更新不自动升级为公开 Release 流程。
- 更新前先核对实际安装路径、运行进程和文件占用；已知宿主会自动拉起插件时，按授权安排一次受控停启，不反复尝试覆盖被占用文件。仅停止已确认属于目标应用的进程，保留其他组件和用户数据；数据库升级前备份，更新后核对安装文件与运行版本。
- 将构建、安装和用户可见生效分别核实，不用“源码已改”代替“本机已更新”。未经明确要求不自动 commit、push 或发布。
- 超出预期耗时、重复失败或需要重打包时，及时说明具体原因、已完成部分和剩余步骤；工作期间至少每 60 秒提供一次有实际信息的简短进度，不能让用户长时间不知道在等什么。不承诺未经验证的完成时间。
