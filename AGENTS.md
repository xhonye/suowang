...
WidBar 个人试用入口位于 `integrations/widbar/`：任务栏优先展示当前模式下一步的最小一步，无最小一步时显示事项名称；原生轻量卡片复用模式、事项和指针 API，支持编辑最小一步、开始／暂停、完成、换一件和空态添加其他事项。它不建立第二份数据库、不读写 SQLite、不增加打卡或催促机制。连接必须验证所选数据目录的实例锁、loopback 监听进程与 health PID；断线隐藏旧事项并明确显示未连接。仅用户点击启动安装版，不后台反复拉起服务。此入口以减少行动成本为目标，不以打开次数为验收目标。
