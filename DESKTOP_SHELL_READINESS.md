...
本文件定义 Windows 与 macOS 桌面壳的长期验收标准，不记录某个本地候选的临时 PASS/BLOCK。Electron 只承担独立窗口与系统集成；页面、Node HTTP 服务、migration 和 SQLite 真源必须继续与浏览器兼容入口共用。

## 用户体验

- 桌面图标直接打开「所往 SUOWANG」独立窗口，没有 PowerShell、终端、浏览器地址栏或 `127.0.0.1`。
- 第二次启动只恢复并聚焦原窗口；正常退出后不残留持有数据库的主进程或 helper。
- Windows Setup、Portable ZIP 与 macOS `.app` 使用同一版本的 packaged app；升级保留用户数据库、备份、头像、设置和窗口状态。
