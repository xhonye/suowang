...
驾驶舱纵向顺序固定为状态 Tab、道路、主线槽、Current 详情、Priority、Todo 双栏。道路是第一视觉焦点，桌面高度约为视口的 25%：1920×1080 约 250–280px，2560×1440 约 320–360px；矮桌面不得低于约 196px。宁可让 Todo 少露一部分，也不能把三岔大道压成横幅。Todo 长列表只在各自列表内部滚动。

320px 下左栏变为底部三页导航，道路保持约 170px 高，双栏垂直排列。信息阅读顺序不变，允许页面滚动，但不允许横向溢出。

## 克制规则

- 三状态共享一套灰蓝系统，不使用绿/蓝/橙三套主题色。
- 道路图基准滤镜为 `saturate(.68) contrast(.94) brightness(1.04)`；雾层只能帮助统一氛围，不能抹掉道路、远景与天空的层次。
- 道路容器使用近乎不可见的边缘和环境阴影，表现为驾驶舱打开的窗，不表现为 dashboard 图片卡片。
