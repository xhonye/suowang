...
[0.2.0-beta.5](https://github.com/xhonye/suowang/releases/tag/v0.2.0-beta.5) 已于 2026-09-08 公开为 Beta，不是稳定版晋级。不可变 Tag `v0.2.0-beta.5` 指向 `a653ba45fe8c8d50fd07f5c075063452ba5b0317`；后续源码或文档变动不改变这批安装包。

- [聚合发布运行](https://github.com/xhonye/suowang/actions/runs/34175378739) 已通过：同 SHA 的完整 CI、Windows Lite/Desktop 与 Mac 候选通过后，在 Draft 集齐并回读校验 8 个附件，再一次性公开。
...
2026-09-08 第一轮 `0.2.0-beta.4` 已发布：下一步为空时可直接「添加第一步」，保存后聚焦「开始这一步」；当天持续事项全部完成时安静收束。第二轮 `0.2.0-beta.5` 已发布：可见事项菜单、触摸与键盘的选择和移动、持续事项当天的选择约束。第三轮 `0.2.0-beta.6` 已通过本地完整发行验证：失败编辑保留、输入法与键盘可靠性、确认弹窗重试、复制和恢复后的模式一致性。完整过程见 [自主迭代记录](autonomous-iterations-2026-09-08.md)。
