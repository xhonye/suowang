...
注册后在 WidBar 的组件列表添加「所往 · 下一步」。组件宽 144 逻辑像素，卡片宽 340、高 370，内容超出时可滚动。卡片关闭由 WidBar 宿主管理。松散包依赖本目录的构建产物，移动源码后需要重新构建并注册；本轮不是公开发行包。

预览文字在标题区域左右居中，最多两行，自适应剩余空间，超出省略、悬停显示完整内容。点击文字打开卡片；右侧无框灰蓝图标直接开始／继续或暂停，进行中显示暂停图标，其他可开始状态显示播放图标；「进行中／暂停中」保留在悬停提示；提交、断线或未保存编辑期间禁用。卡片标题最多两行，开始／暂停为主操作，底部入口固定；编辑后才出现保存／取消。未保存时先处理编辑，再切模式、换事项或执行事项；断线与保存失败仍保留草稿，重试连接后可继续保存。
...
每 3 秒刷新，打开卡片和写入成功后立即刷新；输入最小一步期间暂停定时刷新。任务栏图标表示下一操作，悬停提示当前状态和下一操作，展开卡片以「进行中」显示执行状态，不依赖动画。失败清空可操作旧状态，显示重试入口，不记录事项或 HTTP 正文到日志。所有写入等待服务端确认，不做乐观成功提示。
