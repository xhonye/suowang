...
0.1.1 保留这些尺寸，预览文字自适应剩余空间，悬停显示完整内容。卡片标题最多两行，开始／暂停为主操作，底部入口固定；编辑后才出现保存／取消。未保存时先处理编辑，再切模式、换事项或执行事项；断线与保存失败仍保留草稿，重试连接后可继续保存。
...
每 3 秒刷新，打开卡片和写入成功后立即刷新；输入最小一步期间暂停定时刷新。执行时任务栏右侧常驻「执行中」蓝色标记，展开卡片也显示执行状态，不依赖悬停或动画。失败清空可操作旧状态，显示重试入口，不记录事项或 HTTP 正文到日志。所有写入等待服务端确认，不做乐观成功提示。
